...
AMoonDoorControlPanel::AMoonDoorControlPanel()
{
	...
	// Setup the collider for the moon door control panel
	m_objectCollider->SetWorldScale3D(FVector(0.75f, 1.0f, 0.2f));
	m_objectCollider->SetRelativeLocation(FVector(-235.f, 60.f, 60.0f));
	...
	m_LeftDoor->SetRelativeLocation(FVector(-52.f, 0.f, 0.f));
	...
	m_RightDoor->SetRelativeLocation(FVector(52.f, 0.f, 0.f));
	...
	{
		m_objectMesh->SetStaticMesh(ctrl_panel_mesh.Object);
...
void AMoonDoorControlPanel::ExecuteAnimation(float delta_time)
{
	...
	const float X_CLOSE_POSITION = 50.0f;
